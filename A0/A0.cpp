...
const float SCALE_MOD = 0.5f;

void resetHelper(glm::vec3 &m_shape_color, glm::vec2 &m_shape_translation, float &m_shape_size, float &m_shape_rotation) {
...
void A0::guiLogic()
{
	...
	ImGui::Begin("Shape Properties", &showDebugWindow, ImVec2(100,100), opacity,
		...
		if( ImGui::Button("Reset" ) ) {
            resetHelper(m_shape_color, m_shape_translation, m_shape_size, m_shape_rotation);
		}
        
        // Retrieve color components from slider and store in the first element of
		// m_shape_color.
